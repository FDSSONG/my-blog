...
消息队列（Message Queue, MQ）是一种**非常重要的中间件**技术，广泛应用于分布式系统中，以提高系统的**可用性**、**解耦能力**和**异步通信**效率。

> > ---

电商系统中的"订单支付"场景，订单服务如果直接调用库存、营销等服务，一旦下游某服务故障，订单流程就会阻塞。但引入消息队列后，订单服务只需要把成功支付的消息丢进队列，就算下游服务临时不可以，消息也会在队列里暂存，等待服务恢复后再消费。
...
即一开始订单服务跟下游服务强依赖相关，一个出错阻塞整个流程。且如果新增一个服务（物流服务）必须修改订单服务的代码，扩展性差。引入消息队列后，不再直接调用下游服务，而是消息放入消息队列中；下游服务则从消息队列中“订阅并消费”消息。

解耦的具体价值：

1. 故障隔离：如果库存服务暂时故障，消息会暂存队列中，订单仍能正常响应用户（不会因为下游故障而支付失败），等库存服务恢复后再消费消息。
2. 灵活扩展：如果要新增“物流服务“，只需要让物流服务订阅消息队列中的订单消息即可，无需修改”订单服务“代码。
3. 异步通信：下游可以”异步“处理消息（比如订单支付后，库存扣减、会员积分发放可以在后台慢慢处理），提升系统吞吐量和响应速度。
...
> 分为“削峰”和“填谷”两个核心目标。
>
> 场景：在高并发场景下，用户请求可能在短时间内集中爆发，后端处理服务能力有限。如果把请求直接打给后端，容易压垮服务；“削峰填谷”就是通过中间层把“尖峰”的请求流量削平，再均匀地分发给后端，保障系统稳定。
>
> 如何削峰？1.生产者充当“流量缓冲”，生产者将请求转化为消息，发送给RocketMQ队列，队列就像缓冲区。2.异步响应提升用户体验，不再等后端处理完再响应用户，所以能快速反会结果，用户感知不到请求的高峰的延迟。
>
> 如何填谷？1.消费者控制处理速率，消费者从RocketMQ队列中“按一定速率”读取消息处理。2.动态调整消费速率。根据后端的实际处理能力和负载情况，让请求流量均匀地填充。
...
RocketMQ和Kafka都是十万吞吐量。且消息延迟控制在ms级，ActiveMQ能做到us级。Kafka只支持Java。数据可靠性都非常高，都是分布式架构。消息可靠性都能做到零丢失。RocketMQ功能比Kafka晚辈。
...
1. 单机吞吐量：10w
2. 可用性：分布式架构，非常高。
3. 消息可靠性：经过参数优化，可以零丢失。
4. 功能支持：MQ功能完善，分布式，扩展性好。
5. 支持10亿级别消息堆积，不会因堆积功能下降。
6. 源码是Java，方便二次开发。
7. 为互联网而生，对于可靠性要求高的场景非常有用。
...
JMS是Java Message Service的简称，是一套消息传递规范，定义了在分布式系统下异步通信的API标准，旨在同步不同消息中间件的使用接口。
...
队列模型：队列可以存储多个生产者消息，也可以有多个消费者。但每条消息只能被一个消费者消费。

分布/订阅模型：一份消息分发给多个消费者，每个消费者都收到全量的消息。
...
有消息，主题，标签，消费组，消息队列，消费位置等组件。

1. 消息：传输的具体信息。组成：标识，业务标识，主题标识，可以通过Tag进一步细分同一主题下的消息类型。
2. 主题：消息的一级分类，对应多个消息队列。生产者可以向一/多个主题发送消息。消费者也可订阅一/多个主题。
3. 标签：消息的二级分类，进一步细分，可以用来进行消息过滤。
4. 消费组：不同消费组消费同一个消息主题互不干涉。同一组内的消费者是竞争消费的关系。实现负载均衡和消费可靠性。
5. 消息队列：消息的实际存储单元。
6. 偏移：消费组在某个消息队列中的消费标识。标识哪些被消费过了，哪些没有。
...
### [18.说一下 RocketMQ 的整体工作流程？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_18-说一下-rocketmq-的整体工作流程)
...
### [19.为什么 RocketMQ 不使用 Zookeeper 作为注册中心呢？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_19-为什么-rocketmq-不使用-zookeeper-作为注册中心呢)
...
### [20.Broker 是怎么保存数据的呢？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_20-broker-是怎么保存数据的呢)
...
### [21.说说 RocketMQ 怎么对文件进行读写的？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_21-说说-rocketmq-怎么对文件进行读写的)
...
##### [说说什么是零拷贝?](https://javabetter.cn/sidebar/sanfene/rocketmq.html#说说什么是零拷贝)
...
### [22.消息刷盘怎么实现的呢？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_22-消息刷盘怎么实现的呢)
...
### [23.能说下 RocketMQ 的负载均衡是如何实现的？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_23-能说下-rocketmq-的负载均衡是如何实现的)

RocketMQ 中的负载均衡都在 Client 端完成，具体来说的话，主要可以分为 Producer 端发送消息时候的负载均衡和 Consumer 端订阅消息的负载均衡。

##### [Producer 的负载均衡](https://javabetter.cn/sidebar/sanfene/rocketmq.html#producer-的负载均衡)
...
##### [Consumer 的负载均衡](https://javabetter.cn/sidebar/sanfene/rocketmq.html#consumer-的负载均衡)
...
### [24.RocketMQ 消息长轮询了解吗？](https://javabetter.cn/sidebar/sanfene/rocketmq.html#_24-rocketmq-消息长轮询了解吗)
