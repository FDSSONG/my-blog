...
在 Java 中，垃圾回收机制（Garbage Collection, GC）的触发时机是由 JVM（Java 虚拟机）自动管理 的，开发者无法直接强制触发，但可以通过一些方式 “建议” JVM 进行垃圾回收。具体触发时机和规则如下：

### 一、JVM 自动触发垃圾回收的核心时机

1. 内存不足时（最常见）当 JVM 堆内存不足（如新建对象时无法分配内存），会自动触发垃圾回收，释放不再被引用的对象所占用的内存。
例如：频繁创建大对象导致堆内存接近阈值时，JVM 会触发 GC 腾出空间。

2. 特定分代空间满时（基于分代回收机制）JVM 堆内存通常分为 新生代（Young Generation） 和 老年代（Old Generation），不同区域满时会触发对应的 GC：
    - 新生代满：触发 Minor GC（针对新生代的回收，频率高、速度快）。
    - 老年代满：触发 Major GC/Full GC（针对老年代的回收，可能伴随新生代回收，频率低、速度慢）。
3. 系统空闲时当 JVM 检测到系统资源充足（如 CPU 空闲），可能会主动触发垃圾回收，提前释放内存，避免后续内存紧张。

### 二、开发者可 “建议” 触发 GC 的方式（非强制）
Java 提供了以下方法 “建议” JVM 进行垃圾回收，但 JVM 可以忽略这些建议（是否执行由 JVM 自主决定）：

1. `System.gc()`调用该方法会向 JVM 发出垃圾回收的建议，JVM 可能在当前或稍后执行 GC。
```java
System.gc(); // 建议JVM执行垃圾回收（非强制）
```
2. `Runtime.getRuntime().gc()`与 `System.gc() `功能完全一致，是另一种调用形式：
```java
Runtime.getRuntime().gc(); // 效果同System.gc()
```

### 三、关键注意事项
1. 无法强制触发：即使调用 System.gc()，JVM 也可能因为性能优化（如当前 GC 成本过高）而拒绝执行，因此不能依赖它来确保内存释放。
2. 避免频繁调用：垃圾回收会暂停应用线程（“Stop-The-World”），频繁触发会导致程序响应变慢，影响性能。
3. 回收对象的判定标准：JVM 仅回收 “不可达对象”（即没有任何活跃引用指向的对象），可达对象不会被回收（例如被静态变量、局部变量引用的对象）。

### 总结
Java 垃圾回收的触发时机以 JVM 自动管理 为主（内存不足、分代空间满、系统空闲等），开发者只能通过 System.gc() 等方法 “建议” 触发，但无法强制。实际开发中应避免依赖手动触发，而是通过合理的对象引用管理（如及时释放无用引用）优化 GC 效率。
...
![Snipaste_2025-11-03_17-50-34](./image/JVM/Snipaste_2025-11-03_17-50-34.png)

JVM的内存结构主要分为以下几个部分：

- 程序计数器：可以看作是**当前线程**所执行的字节码的行号指示器，用于存储当前线程正在执行的 Java 方法的 JVM 指令地址。如果线程执行的是 Native 方法，计数器值为 null。是唯一一个在 Java 虚拟机规范中没有规定任何 OutOfMemoryError 情况的区域，生命周期与线程相同。
- Java 虚拟机栈：**每个线程**都有自己独立的 Java 虚拟机栈，生命周期与线程相同。每个方法在执行时都会创建一个栈帧，用于存储局部变量表、操作数栈、动态链接、方法出口等信息。可能会抛出 StackOverflowError 和 OutOfMemoryError 异常。
...
这里的关键点是，栈中存储的不是对象，而是对象的引用。也就是说，当你在方法中声明一个对象，比如`MyObject obj = new MyObject();`，这里的obj实际上是一个存储在栈上的引用，指向堆中实际的对象实例。这个引用是一个固定大小的数据（例如在64位系统上是8字节），它指向堆中分配给对象的内存区域。


## 1.4 堆分为哪几部分呢？
Java堆（Heap）是Java虚拟机（JVM）中内存管理的一个重要区域，主要用于存放对象实例和数组。随着JVM的发展和不同垃圾收集器的实现，堆的具体划分可能会有所不同，但通常可以分为以下几个部分：

![Snipaste_2025-11-03_18-02-19](./image/JVM/Snipaste_2025-11-03_18-02-19.png)
...
## 1.10 String s = new String（“abc”）执行过程中分别对应哪些内存区域？
...
- 强引用指的就是代码中普遍存在的赋值方式，比如`A a = new A()`这种。强引用关联的对象，永远不会被GC回收。
...
- 直接内存内存溢出：在使用`ByteBuffer`中的`allocateDirect()`的时候会用到，很多`JavaNIO(像netty)`的框架中被封装为其他的方法，出现该问题时会抛出`Java.lang.OutOfMemoryError: Direct buffer memory`异常。


## 1.15 有具体的内存泄漏和内存溢出的例子么请举例及解决方案?
```
1、静态属性导致内存泄露
```
...
![Snipaste_2025-11-03_18-24-10](./image/JVM/Snipaste_2025-11-03_18-24-10.png)


但针对上述程序，如果将定义list的变量前的static关键字去掉，再次执行程序，会发现内存发生了具体的变化。VisualVM监控信息如下图：


![Snipaste_2025-11-03_18-25-49](./image/JVM/Snipaste_2025-11-03_18-25-49.png)
...
![Snipaste_2025-11-03_18-28-50](./image/JVM/Snipaste_2025-11-03_18-28-50.png)
...
```
ThreadLocal这个例子没看懂，得具体看ThreadLocal实现
```
# 二、类初始化和类加载
## 2.1 创建对象的过程？

![Snipaste_2025-11-03_18-34-52](./image/JVM/Snipaste_2025-11-03_18-34-52.png)
...
![Snipaste_2025-11-03_18-40-29](./image/JVM/Snipaste_2025-11-03_18-40-29.png)
...
![Snipaste_2025-11-03_18-42-51](./image/JVM/Snipaste_2025-11-03_18-42-51.png)
...
- 卸载：一个类要被JVM卸载，条件非常苛刻，需要同时满足以下三点：
    ...
    -*+-- 类对应的Java.lang.Class对象没有任何地方被引用：不能在任何地方通过反射（如静态字段、全局变量）、静态变量、JNI等途径引用   到这个Class对象。一旦这个Class对象还存在强引用，GC就不会回收它，那么这个类也就不会被卸载。
...
![Snipaste_2025-11-03_18-52-32](./image/JVM/Snipaste_2025-11-03_18-52-32.png)
...
![Snipaste_2025-11-03_18-53-43](./image/JVM/Snipaste_2025-11-03_18-53-43.png)
...
![Snipaste_2025-11-03_18-55-04](./image/JVM/Snipaste_2025-11-03_18-55-04.png)
...
## 3.8 minorGC、majorGC、fullGC的区别，什么场景触发full GC
...
![Snipaste_2025-11-03_18-58-35](./image/JVM/Snipaste_2025-11-03_18-58-35.png)
